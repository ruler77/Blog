...
module.exports = class Article extends require('./model') {
    ...
    static getList() {
        return new Promise((resolve, reject) => {
            let sql = 'SELECT id,title,content,time,hits,user_id FROM article ORDER BY TIME DESC'
     ...
     * 获取文章列表不返回内容
     */
    static getListWithoutContent() {
        return new Promise((resolve, reject) => {
            let sql = 'SELECT id,title,time,hits,user_id FROM article ORDER BY TIME asc'
            this.query(sql).then(results => {
                resolve(results)
            }).catch(err => {
                console.log(`获取文章列表不返回内容失败：${err.message}`)
                reject(err)
            })
        })
    }
    ...
    static getListByCategoryId(id) {
        return new Promise((resolve, reject) => {
            let sql = 'SELECT id,title,content,time,hits,user_id FROM article WHERE category_id = ? ORDER BY TIME DESC'
    ...
    /**
     * 获取指定标签下的文章列表
     * @param {integer}} id 标签编号
     */
    static getListByTagId(id) {
        return new Promise((resolve, reject) => {
            let sql = 'select * from article where id in (select articleid from article_tag where tagid= ? ) ORDER BY TIME DESC'
            this.query(sql, id).then(results => {
                resolve(results)
            }).catch(err => {
                console.log(`获取指定标签下的文章列表失败：${err.message}`)
                reject(err)
            })
        })
    }
    ...
    static getArticleById(id) {
        return new Promise((resolve, reject) => {
            let sql = 'SELECT a.id,a.title,a.content,a.`time`,a.hits,a.`category_id`,c.`name`,a.`thumbnail`,\
            a.`hot`,a.user_id FROM article a,category c WHERE a.`category_id` = c.`id` AND a.id = ?'
            this.query(sql, id).then(results => {
                resolve(results[0])
            }).catch(err => {
